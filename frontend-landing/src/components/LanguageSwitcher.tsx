...
export function LanguageSwitcher({ lang, onChange, dir = "ltr" }: LanguageSwitcherProps) {
  ...
  return (
    <DropdownMenu dir={dir}>
      <DropdownMenuTrigger
        className="glass touch-target group flex h-10 cursor-pointer items-center gap-1.5 rounded-full px-3 font-display text-sm font-medium text-foreground/85 transition-all duration-300 hover:-translate-y-0.5 hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        aria-label={current.native}
        title={current.native}
      >
        <FlagIcon lang={current.code} />
        <span className="font-medium">{current.label}</span>
        <ChevronDown aria-hidden="true" className="h-3.5 w-3.5 text-muted-foreground transition-transform duration-300 group-data-[state=open]:rotate-180" />
      </DropdownMenuTrigger>
      <DropdownMenuContent
          ...
          return (
            <DropdownMenuItem
              ...
              className={`flex cursor-pointer items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm transition-colors ${
                active
                  ? "bg-brand-orange/10 font-semibold text-accent-ink focus:bg-brand-orange/15 focus:text-accent-ink"
                  : "text-foreground/80"
              }`}
            >
